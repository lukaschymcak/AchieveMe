...
const poisoned = {}
for (const [depotId] of Object.entries(live)) {
  poisoned[depotId] = '000000000000000001'
}

const db = new Database(dbPath)
// undo accidental Stellar Blade seed
db.prepare("UPDATE games SET manifest_gids = '', update_status = '' WHERE appid = '3489700'").run()
db.prepare(
  "UPDATE games SET manifest_gids = ?, update_status = '' WHERE appid = ?"
).run(JSON.stringify(poisoned), APPID)

const row = db.prepare('SELECT appid, name, manifest_gids, update_status FROM games WHERE appid = ?').get(APPID)
console.log('POISONED_UNPACKING', row)
db.close()
