...
**Update checks:** After window ready-to-show (and again on global **Refresh**), AchieveMe anonymously queries Steam PICS (via `steam-user`) for every game with stored GIDs. Library cards show an ↑ badge when an update is available **only for games with stored depot GIDs**. Game Detail shows Update / Validate / Check for update and build status **only when GIDs are stored** (Depot Downloader download or Import existing) — achievement-scan-only games hide that chrome. **Update** and **Validate** always open an inline depot picker (no auto-select of macOS/Windows depots) — depots matching stored GIDs are pre-ticked, but you confirm the selection. Update fetches a fresh Hubcap ZIP, then **replaces** stored `manifest_gids` with GIDs for the selected depots only. Validate uses `manifest_cache/{appid}.zip` when present and does not change GIDs. Both run DepotDownloader with `-validate` against the resolved game root (`resolveGameRoot`, never a deep DLL folder). Validate appears whenever GIDs are stored; if `install_path` is missing it prompts for a folder first. Progress is lifted to the app shell so leaving Game Detail and returning still shows the in-flight job; the session clears as soon as the IPC job finishes (no stuck 100% state). Network failures leave the previous status unchanged. After a successful **Update**, if Steamless and/or Goldberg were previously applied for that game (`steamless_applied` / `goldberg_applied`), AchieveMe asks whether to reapply them — you confirm paths and press Apply; tools run sequentially (Steamless then Goldberg), stop on the first failure, and offer Retry. Nothing is auto-applied. Flags are set only when those tools succeed going forward; existing library rows stay unset until you use them again.
