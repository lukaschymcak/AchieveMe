...
**Scan for installed games:** Tools chip next to Import existing. Configure roots under **Settings → Install scan folders** (not Custom Watch Folders). Scan finds `steam_appid.txt` or numeric folders with `steam_api*.dll`, then checklist-add sets `install_path` (and a ranked suggested exe when found). No Hubcap GIDs and no fake achievements — set those up from Game Detail later.
...
9. **Scan installed** — Settings → Install scan folders → add a test folder with `steam_appid.txt` (or numeric folder + `steam_api64.dll`). Tools → Scan for installed games → Scan → select → Add selected. Library row appears; Refresh does not delete it; no fake achievements. Import existing still requires Hubcap GIDs.
