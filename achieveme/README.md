...
AchieveMe can back up **library games only** through a user-linked [Ludusavi](https://github.com/mtkennerly/ludusavi) CLI (`ludusavi.exe` — not bundled). Link the path under **Settings → Save backups**, optionally enable **Auto-backup library saves** (startup / after play session / when adding a game), or click **Backup all library games now** (backup-only; no bulk restore). On Game Detail, maintenance actions sit left of **Refresh all**; the **floppy** icon opens **Back up saves** or **Install backup** (Ludusavi restore — overwrites current save files) and shows a spinner while a job runs. Matching uses `ludusavi find --steam-id <appid> --api`, then `backup` or `restore` with `--force --api --no-cloud-sync`. One Ludusavi process runs at a time. Backups live in Ludusavi’s own folders — not AchieveMe’s SQLite.
