...
**Playtime** counts while the game’s **install folder** or **Play path** is running. Matching uses the **full path** (not process name), about every **2 seconds**; playtime saves about every **30 seconds** and on quit. Optionally **hide to tray when a game starts** (Settings → Tray & startup). Dashboard, game detail, and library always show tracked playtime (or `—`). On the Emulator step you can mark a game as **Denuvo offline activated** so AchieveMe keeps existing `configs.user.ini`, `configs.overlay.ini`, `configs.app.ini`, and `configs.main.ini` when replacing `steam_settings` (back up that folder first if it already exists).
...
**Depot Downloader:** Search Steam games, download a Hubcap manifest ZIP (`hubcapmanifest.com`), pick depots, and run `DepotDownloader.dll` via `dotnet`. Configure an optional Hubcap API key and default download folder under **Settings → Depot Downloader**. Binaries live in `achieveme/resources/depotdownloader/` (bundled as `extraResources` on package). Closing the wizard during a download keeps it running; the app-shell **Transfers** dock lists the job and reopens the wizard with live progress from any page. As soon as the download finishes, AchieveMe stores **only the selected depots'** manifest GIDs plus the download folder as `install_path` — the game appears in the library immediately (even at 0 achievements). You can set up Goldberg achievements in the wizard, or click **Skip** (skips Goldberg only; the library row and GIDs stay). Refresh does not prune depot installs that have stored GIDs or an `install_path`. Game Detail shows **Set up achievements** when `total_achievements` is 0.

**Scan for installed games:** Tools chip next to Import existing. Configure roots under **Settings → Install scan folders** (not Custom Watch Folders). Scan walks each root up to depth 8 for `steam_appid.txt` or numeric folders with `steam_api*.dll`, then **Add selected** sets `install_path` (and a ranked suggested exe when found) and pulls Hubcap manifest GIDs — Windows and DLC depots auto-keep; Linux/mac depots are dropped; ambiguous depots open a picker queue. Does **not** run Set up achievements — use Game Detail when you want Goldberg setup. Refresh keeps rows that have an install path.
**Import existing folder:** On Tools → Depot Downloader, **Import existing folder** registers a game already on disk without running DepotDownloader. It fetches the Hubcap manifest only, you browse to the install folder and tick the depots you have, then AchieveMe writes `manifest_gids` + `install_path` so Update / Validate / Check for update work.

## Save backups (Ludusavi)

AchieveMe can back up **library games only** through a user-linked [Ludusavi](https://github.com/mtkennerly/ludusavi) CLI (`ludusavi.exe` — not bundled). Link the path under **Settings → Save backups**, optionally enable **Auto-backup library saves** (startup / after play session / when adding a game), or click **Backup all library games now** (backup-only; no bulk restore). On Game Detail, maintenance actions sit left of **Refresh all**; the **floppy** icon opens **Back up saves** or **Install backup**. Backups from AchieveMe pass `--full-limit 5` so Ludusavi retains up to five full snapshots for that title **when save files actually change** (identical saves do not create a new snapshot — AchieveMe shows an unchanged note). **Install backup** lists those snapshots from live `ludusavi backups --api` (newest first, at most five shown) so you pick which point to restore; restore uses `--backup <id>` and overwrites current save files. Matching uses `ludusavi find --steam-id <appid> --api`, then `backup` or `restore` with `--force --api`. Backups use `--cloud-sync` only when **Upload after backup** is enabled; otherwise `--no-cloud-sync`. Restore / list / Install picker always stay local (`--no-cloud-sync`). One Ludusavi backup/restore process runs at a time (listing snapshots may briefly spawn a second process). Retention and backup folders stay in Ludusavi — AchieveMe only stores status timestamps in SQLite.

**Cloud (rclone):** Optionally link [rclone](https://rclone.org/) (`rclone.exe`), pick Google Drive / OneDrive / Dropbox / Box / a custom rclone remote, and **Connect** (`ludusavi cloud set …`). AchieveMe always passes `--config` pointing at `%APPDATA%\achieveme\ludusavi` (or the app’s `userData/ludusavi`) so your Ludusavi **GUI** `config.yaml` is never rewritten. If local and cloud disagree, AchieveMe shows a soft conflict note and **never** auto-resolves — use **Upload to cloud** / **Download from cloud** in Settings (overwrite confirm). FTP/SMB/WebDAV: create the remote with `rclone config`, then choose Custom.

**Delete and ignore:** Library Delete removes the SQLite row and Goldberg/GSE save folders, then adds the AppID to an `ignored_appids` table so leftover CODEX/RUNE saves do not re-add the title on Refresh. CODEX/RUNE files are never deleted. Add Game, Depot download, or Import existing clears the ignore.
**Update checks:** After window ready-to-show (and again on global **Refresh**), AchieveMe anonymously queries Steam PICS (via `steam-user`) for every game with stored GIDs. Library cards show an ↑ badge when an update is available **only for games with stored depot GIDs**. Game Detail shows Update / Validate / Check for update and build status **only when GIDs are stored** (Depot Downloader download or Import existing) — achievement-scan-only games hide that chrome. **Update** and **Validate** open the app-shell **Update transfer modal** (depot pick → live progress). Closing the modal never cancels the job — the **Transfers** dock stays visible on every page and reopens the modal. Update fetches a fresh Hubcap ZIP, then **replaces** stored `manifest_gids` with GIDs for the selected depots only. Validate uses `manifest_cache/{appid}.zip` when present and does not change GIDs. Both run DepotDownloader with `-validate` against the resolved game root (`resolveGameRoot`, never a deep DLL folder). Validate appears whenever GIDs are stored; if `install_path` is missing it prompts for a folder first. Network failures leave the previous status unchanged. After a successful **Update**, if Steamless and/or Goldberg were previously applied (`steamless_applied` / `goldberg_applied`), reapply is a **phase of the Update modal** (not a separate Game Detail prompt) so navigating away mid-download cannot drop it — you confirm paths and Apply; tools run sequentially (Steamless then Goldberg), stop on the first failure, and offer Retry. Validate never enters reapply. Nothing is auto-applied. Flags are set only when those tools succeed going forward; existing library rows stay unset until you use them again.
...
14. **Playtime** — Add a game with install folder / Play path, enable Track playtime, launch the game. Within ~2s AchieveMe should detect it; after ~30s open Library (or Tray → Show) and confirm playtime increased without quitting. Quit the game → playtime finalizes; session recap appears if ≥1 minute. Kill AchieveMe mid-session then relaunch with the game already closed → playtime kept, no surprise recap. With **Hide to tray when a game starts** enabled → Save → Play → main window hides; restores when the session ends.
