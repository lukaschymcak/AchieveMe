...
AchieveMe can back up **library games only** through a user-linked [Ludusavi](https://github.com/mtkennerly/ludusavi) CLI (`ludusavi.exe` — not bundled). Link the path under **Settings → Save backups**, optionally enable **Auto-backup library saves** (startup / after play session / when adding a game), or click **Backup all library games now**. Game Detail shows a live status pill (**Backing up…** / **Saves backed up** / **Not in Ludusavi** / **Backup failed**) and **Backup now**. Matching uses `ludusavi find --steam-id <appid> --api`, then `ludusavi backup --force --api --no-cloud-sync`. One Ludusavi process runs at a time. Backups live in Ludusavi’s own folders — not AchieveMe’s SQLite. In-app restore is not available yet; use Ludusavi to restore.
