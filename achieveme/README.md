...
**Playtime** is tracked for games with an **install folder** and/or **launch exe**. AchieveMe matches the running process by path/PID about every **2 seconds** (not by process name alone), writes playtime about every **30 seconds** while the game runs, and flushes on session end or quit so a crash no longer loses the whole session. Optionally **hide to tray when a tracked game starts** (Settings → Tray & startup). Dashboard, game detail, and library cards/rows always show tracked playtime (or `—` when none yet). On the Emulator step you can mark a game as **Denuvo offline activated** so AchieveMe keeps existing `configs.user.ini`, `configs.overlay.ini`, `configs.app.ini`, and `configs.main.ini` when replacing `steam_settings` (back up that folder first if it already exists).
...
13. **Playtime** — Add a game with install folder / launch exe, enable Track playtime, launch the game. Within ~2s AchieveMe should detect it; after ~30s open Library (or Tray → Show) and confirm playtime increased without quitting. Quit the game → playtime finalizes; session recap appears if ≥1 minute. Kill AchieveMe mid-session then relaunch with the game already closed → playtime kept, no surprise recap. With **Hide to tray when a tracked game starts** enabled → Save → Play → main window hides; restores when the session ends.
