...
import { manifestFilePath } from './manifestZipService'

const execFileAsync = promisify(execFile)
const DOWNLOADS_PROGRESS_CHANNEL = 'depot:progress'
...
/**
 * Resolves the packaged or dev resources root for DepotDownloader binaries.
 */
export function depotResourcesRoot(): string {
  if (app.isPackaged) {
    return path.join(process.resourcesPath, 'depotdownloader')
  }
  return path.resolve(app.getAppPath(), 'resources', 'depotdownloader')
}

/**
 * Absolute path under the DepotDownloader resources folder.
 */
export function depPath(...parts: string[]): string {
  return path.join(depotResourcesRoot(), ...parts)
}

/**
 * Locates a usable `dotnet` executable.
 */
export function findDotnet(): string | null {
  ...
  return candidates.find((candidate) => candidate === 'dotnet' || fs.existsSync(candidate)) || null
}

/**
 * Starts DepotDownloader for the selected depots and streams progress to the renderer.
 */
export async function startDownload(
  request: DepotDownloadStartRequest,
  window: BrowserWindow
): Promise<void> {
  const dotnet = findDotnet()
  if (!dotnet) throw new Error('dotnet not found. Install the .NET runtime to use DepotDownloader.')
  const depotDll = depPath('DepotDownloader.dll')
  if (!fs.existsSync(depotDll)) {
    throw new Error(
      `DepotDownloader.dll not found at ${depotDll}. Copy LuDownloader resources into achieveme/resources/depotdownloader.`
    )
  }

  const outputDir =
    request.outputPath ||
    path.join(request.libraryPath, 'steamapps', 'common', safeInstallDir(request.gameData))
  await fsp.mkdir(outputDir, { recursive: true })
  const depotKeys = await writeDepotKeys(request.gameData)

  const depots = request.selectedDepots.length
    ? request.selectedDepots
    : Object.keys(request.gameData.depots)
  ...
  const tempManifestFiles = depots
    .map((depotId) =>
      request.gameData.manifests[depotId]
        ? manifestFilePath(depotId, request.gameData.manifests[depotId])
        : ''
    )
    .filter((v) => Boolean(v))

  const state = createOrResetActive(
    request.channelId,
    depots,
    depotSizes,
    totalBytes,
  ...
  try {
    for (let index = 0; index < depots.length; index++) {
      ...
      pushLog(
        ...
        window,
        emit,
        `--- Depot ${index + 1}/${depots.length}: ${depotId} ---`,
        true
      )
      ...
      const args = [
        depotDll,
        '-app',
        request.gameData.appId,
        '-depot',
        depotId,
        '-manifest',
        manifest,
        '-manifestfile',
        manifestFilePath(depotId, manifest),
        '-depotkeys',
        depotKeys,
        '-max-downloads',
        String(request.maxDownloads || 20),
        '-dir',
        outputDir,
        '-validate'
      ]
    ...
    stopTelemetryLoop(state)
    await fsp.unlink(depotKeys).catch(() => undefined)
...
/**
 * Cancels an active DepotDownloader session.
 */
export async function cancelDownload(
  channelId: string,
  mode: DepotCancelMode = 'keep'
): Promise<void> {
    ...
    proc.on('close', (code) => {
      ...
      if (code === 0) resolve()
        ...
        pushLog(
          ...
          window,
          emit,
          `WARNING: DepotDownloader exited with code ${code}`,
          true
        )
...
async function writeDepotKeys(gameData: GameData): Promise<string> {
  const filePath = path.join(
    os.tmpdir(),
    `achieveme_depotkeys_${gameData.appId}_${Date.now()}.txt`
  )
...
function updateHealthFromLine(state: ActiveDownload, line: string): void {
  ...
  const now = Date.now()
  state.runtime.retryTimestamps = state.runtime.retryTimestamps.filter(
    (t) => now - t <= RECENT_WINDOW_MS
  )

  if (
    lower.includes('connection timeout downloading chunk') ||
  ...
  if (
    ...
    state.runtime.healthState !== 'warning' &&
    state.runtime.retryTimestamps.length === 0
  ) {
...
function startTelemetryLoop(
  ...
  state.runtime.telemetry.interval = setInterval(() => {
    void (async () => {
      ...
      const rxDelta =
        rxBytes === null ? 0 : Math.max(0, rxBytes - live.runtime.telemetry.lastRxBytes)
      ...
      live.runtime.telemetry.smoothSpeedBps =
        prev > 0 ? prev * 0.7 + instantSpeed * 0.3 : instantSpeed

      const currentBytes =
        live.runtime.currentDepotBytes * (live.runtime.telemetry.knownPct / 100)
      ...
      const etaSec =
        !live.paused && live.runtime.telemetry.smoothSpeedBps > 32
          ? Math.round(remaining / live.runtime.telemetry.smoothSpeedBps)
          : null
      const pct =
        live.runtime.totalBytes > 0
          ? Math.max(
              0,
              Math.min(100, (downloadedBytesEstimate / live.runtime.totalBytes) * 100)
            )
...
async function getGlobalReceivedBytes(): Promise<number | null> {
  ...
  const script =
    '(Get-NetAdapterStatistics -ErrorAction SilentlyContinue | Measure-Object -Property ReceivedBytes -Sum).Sum'
  try {
...
async function cleanupCanceledArtifacts(state: ActiveDownload): Promise<void> {
  await fsp.rm(state.runtime.outputDir, { recursive: true, force: true }).catch(() => undefined)
  await Promise.all(
    state.runtime.tempManifestFiles.map((m) => fsp.rm(m, { force: true }).catch(() => undefined))
  )
}
