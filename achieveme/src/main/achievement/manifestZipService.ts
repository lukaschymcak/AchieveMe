...
/**
 * Parses a Hubcap manifest ZIP into GameData and extracts .manifest sidecars.
 *
...
export async function processZip(zipPath: string): Promise<GameData> {
  ...
  const tempDir = path.join(os.tmpdir(), 'achieveme_manifests')
...
/**
 * Absolute path to a sidecar .manifest file extracted from a Hubcap ZIP.
 */
export function manifestFilePath(depotId: string, manifestGid: string): string {
  return path.join(os.tmpdir(), 'achieveme_manifests', `${depotId}_${manifestGid}.manifest`)
}
...
function findSingleLuaEntry(entries: ZipEntry[], requireSingle = false): ZipEntry | undefined {
  const luaEntries = entries.filter(
    (entry) => !entry.isDirectory && entry.entryName.toLowerCase().endsWith('.lua')
  )
