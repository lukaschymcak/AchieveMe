...
export const HELP_SECTIONS: HelpSection[] = [
  ...
  },
  {
    ...
    title: 'Notifications',
    paragraphs: [
      'When a save file changes and a new achievement unlocks, AchieveMe shows a Hydra-style toast (centered icon hold → expand from center to measured width → hold → shrink to icon → icon hold → fade). White title + description, rarity border glow; gold shine / teal platinum chrome. Unlock toasts use cached achievement icons (`achieveme-img://`); if the schema has no icon or the cache cannot serve one, the toast still appears with a fallback glyph. When a game first reaches 100%, a platinum celebration toast follows. Library Refresh and first launch never spam toasts for existing unlocks. Use Settings → Test notification to cycle through all four skins.',
