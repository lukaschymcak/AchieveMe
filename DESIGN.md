...
- **Chrome header** (`.library-chrome`): flex row, align center, gap `var(--space-3)`, padding-bottom `var(--space-4)`, border-bottom `1px solid var(--border-subtle)`. Slots: left (nav), center (Wanted rail), right (meta + actions). Library has **no** center search field; the center slot is a compact parallelogram Wanted rail with Store-cached covers (`achieveme-img://`) and chip actions (**Add to Library** via + or context menu). Sort chips and launcher toggle live on `.library-chrome__toolbar`; toolbar **+** is always Add Game (Goldberg setup).
