...
- **Chrome header** (`.library-chrome`): flex row, align center, gap `var(--space-3)`, padding-bottom `var(--space-4)`, border-bottom `1px solid var(--border-subtle)`. Slots: left (nav), center (Wanted rail), right (meta + actions). Library has **no** center search field; the center slot is the Wanted rail in the chrome search slot — a 36px `--radius-sm` search well with compact list-row chips (Store-cached covers), hover-expand names, and chip actions (**Add to Library** via + or context menu). Sort chips and launcher toggle live on `.library-chrome__toolbar`; toolbar **+** is always Add Game (Goldberg setup).
- **Responsive:** wraps at 1100px (nav full width, right row below); sort divider drops at 640px.
